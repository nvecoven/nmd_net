...
class Fakenet():
    ...
    def save_models(self, t = 0):
        return

    def get_action(self, current_steps, state = False):
        return np.array([[0.5] for el in current_steps]), 0, 0

    def get_value(self, trajectories):
        return np.ones(len(trajectories))

    def update_policy(self, policy_trajectories, nbr_epochs=2,
                         batch_size=1,
                         check_early_stop=2):
        return

    def update_value_function(self, value_trajectories, nbr_epochs=1,
                                 batch_size=None):
        return
