...
class DeterministMultipleReferences():
    def __init__(self, number_dots = 3, nbr_good = 1, fixed_good_refs = True, control_speed = False,
        ...
        self.nofb_obs_dim = self.number_dots * 2 + 1 # pos_x, pos_y, timestep
        if self.stop_input:
            self.nofb_obs_dim += 1
        ...
        self.default_pos = [0.0, -1.5 * self.max_size]
    ...
    def create_references(self):
        ...
        for r in range(self.number_dots):
            created = False
            while not created:
                random_pos = [-self.max_size + 2 * r * self.max_size,
                              0.5 * self.max_size +  0.3 * r * self.max_size, r<self.nbr_good_ref]
                distances = []
